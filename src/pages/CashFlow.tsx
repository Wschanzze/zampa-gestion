        ...
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-4 py-3 text-right bg-indigo-50 text-indigo-900 font-bold">TOTAL</th>
            </tr>
          </thead>
          <tbody>
            {/* Ingresos Section */}
            ...
            {rubrosIngreso.map(rubro => (
              <tr key={rubro} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-4 py-2 text-gray-700">{rubro}</td>
                {sortedMonths.map(m => (
                  <td key={m} className="px-4 py-2 text-right">{formatCurrency(matrix[rubro]?.[m] || 0)}</td>
                ))}
                <td className="px-4 py-2 text-right font-medium bg-indigo-50/50">{formatCurrency(calculateRowTotal(rubro))}</td>
              </tr>
            ))}
            <tr className="border-b-2 border-emerald-200 bg-emerald-50">
              ...
              <td className="px-4 py-2 text-right font-bold text-emerald-900 bg-indigo-100/50">
            ...
            <tr className="border-b-2 border-rose-200 bg-rose-50">
              ...
              <td className="px-4 py-2 text-right font-bold text-rose-900 bg-indigo-100/50">
            ...
            {/* SALDO MENSUAL */}
            <tr className="bg-indigo-100 border-t border-b border-indigo-200">
              <td className="px-4 py-3 font-bold text-indigo-900 uppercase">Flujo de Caja Mensual</td>
              {sortedMonths.map(m => {
              ...
              <td className="px-4 py-3 text-right font-bold text-indigo-900 bg-indigo-200">
                {formatCurrency(
